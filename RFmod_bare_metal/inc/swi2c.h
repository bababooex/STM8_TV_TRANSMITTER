...
4.6.2024 - removed function function swi2c_write_array_wctrbyte() - duplicate of swi2c_write_buf()
...
#define SCL_stat() ((PORT(SCL_GPIO, IDR) & SCL_PIN) ? 1 : 0)
#define SDA_stat() ((PORT(SDA_GPIO, IDR) & SDA_PIN) ? 1 : 0)

#define SCL_HIGH (PORT(SCL_GPIO, ODR) |= SCL_PIN) 
#define SCL_LOW (PORT(SCL_GPIO, ODR) &= ~SCL_PIN)  
#define SDA_HIGH (PORT(SDA_GPIO, ODR) |= SDA_PIN)  
#define SDA_LOW (PORT(SDA_GPIO, ODR) &= ~SDA_PIN) 

#define SWI2C_SS_TIME delay_us(SWI2C_START_STOP_TIME)
#define SWI2C_SETUP_TIME delay_us(SWI2C_SDA_SETUP_TIME)
#define SWI2C_HOLD_TIME delay_us(SWI2C_SDA_HOLD_TIME)
#define SWI2C_SCL_HIGH_TIME delay_us(SWI2C_SCL_HALFPERIOD_TIME)

// user functions
void swi2c_init(void); // init GPIOs as Open Drain outputs
uint8_t swi2c_test_slave(uint8_t slvaddr); // check if slave with selected addres is present on bus
...
//experimental
uint8_t swi2c_read_array(uint8_t slv_addr, uint8_t* data, uint16_t num); // SLA+W - num*1B(data)
